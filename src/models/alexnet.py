...
class AlexNet(nn.Module):
    ...
    def train_model(self, train_loader, val_loader, idx2lbl, device, *, epochs=10, lr=1e-3):
        self.to(device)
    ...
    def _report(self, y_true, y_pred, idx2lbl, name):
        ...
        plt.tight_layout()
        plt.savefig(f"confusion_matrix_alexnet_{name.lower()}.png")
        plt.close()

        print("\n".join(report))
        with open("relatorio_resultado_alexnet.txt", "a") as f:
            f.write("\n".join(report) + "\n\n")
