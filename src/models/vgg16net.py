...
class VGG16Net(nn.Module):
    ...
    def forward(self, x):
        B, T, H, W = x.shape
        x = x.unsqueeze(2).view(B * T, 1, H, W)
        x = x.repeat(1, 3, 1, 1)
    ...
    def train_model(self, train_loader, val_loader, idx2lbl, device, *, epochs=20, lr=1e-4):
        self.to(device)
        ...
        # Usando um scheduler para reduzir o learning rate quando a perda não melhorar
        ...
        for ep in range(1, epochs + 1):
            ...
            # Atualizar o scheduler com a média da perda
    ...
    def _report(self, y_true, y_pred, idx2lbl, name):
        ...
        plt.tight_layout()
        plt.savefig(f"confusion_matrix_vgg16net_{name.lower()}.png")
        plt.close()

        print("\n".join(report))
        with open("relatorio_resultado_vgg16net.txt", "a") as f:
            f.write("\n".join(report) + "\n\n")
